...
def test_get_access_method():
    assert json.dumps(drs_utils.get_access_method_of_type(TEST_DRS_REPLY, "file"), sort_keys=True) == json.dumps(
        TEST_DRS_REPLY["access_methods"][0], sort_keys=True
    )
...
def test_drs_uri_fetch_sync():
    responses.add(
        responses.GET, f"https://example.org/ga4gh/drs/v1/objects/{TEST_DRS_ID}", json=TEST_DRS_REPLY, status=200
    )

    assert json.dumps(
        drs_utils.fetch_drs_record_by_uri(f"drs://example.org/{TEST_DRS_ID}"), sort_keys=True
...
def test_drs_uri_fetch_sync_errors():
    responses.add(
        ...
        json=errors.not_found_error(drs_compat=True),
        status=404,
    )
    responses.add(
        responses.GET,
        "https://example.org/ga4gh/drs/v1/objects/xyz",
        json=errors.not_found_error(drs_compat=True),
        status=400,
    )
