...
class WorkflowSet:
    ...
    def workflow_dicts_by_type_and_id(self) -> dict[str, dict[str, WorkflowDefinition]]:
        by_type: dict[str, dict[str, WorkflowDefinition]] = defaultdict(dict)
        for id_, wf in self._defs_by_id.items():
            by_type[wf.type][id_] = cast(WorkflowDefinition, wf.model_dump(mode="json"))
        return dict(by_type)

    def workflow_dicts_by_id(self) -> dict[str, WorkflowDefinition]:
        return {k: cast(WorkflowDefinition, v.model_dump(mode="json")) for k, v in self._defs_by_id.items()}
