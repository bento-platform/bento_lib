...
class DiscoveryConfig(BaseModel, NoAdditionalProperties):
    ...
    catalogue_charts: list[OverviewChart] = Field(
        default_factory=list,
        title="Catalogue charts",
        description=(
            "Chart field IDs to show in the data catalogue view (if enabled.) This only applies for an instance-level "
    ...
    def _check_catalogue_field_references(self):
        """
        ...
        for c_idx, chart in enumerate(self.catalogue_charts):
            exc_path = f"catalogue_charts > {chart.field} {chart.chart_type} [{c_idx}]"
            log_data = dict(field=chart.field, chart_idx=c_idx)
    ...
    def check_field_references(self) -> Self:
        self._check_catalogue_field_references()
    ...
    def get_chart_field_ids(self, catalogue_mode: bool = False) -> tuple[str, ...]:
        ...
        """
        if catalogue_mode:
            return tuple(chart.field for chart in self.catalogue_charts)
